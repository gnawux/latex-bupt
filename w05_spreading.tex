...
\begin{eqnarray}
	F(t_1<t_2) & = & \int_{-\infty}^{\infty}\int_{-\infty}^{t_2}f(t_1)f(t_2) dt_1 dt_2 \nonumber\\
	           & = & \int_{T_{u0}}^{T_{u0}+T_r\cdot r_0}\!\int_{T_{u0}}^{t_2}\frac{1}{T_r\cdot r_0}\cdot\frac{1}{T_r\cdot\left(r_0+r\right)}dt_1 dt_2 + \int_{T_{u0}+T_r\cdot r_0}^{T_{u0}+T_r\cdot\left(r_0+r\right)}\frac{1}{T_r\cdot\left(r_0+r\right)}dt_2\nonumber\\
			   & = & \frac{r_0+2r}{2(r0+r)}
	\label{eq:proplt1}
\end{eqnarray}
