...
然而，对于移动个人网络来说，结构化和非结构化的~P2P~方式均有其各自的不足：无中心的~P2P~分布式系统通常用于资源共享，没有固定的存储机制，不存在一个确定的~``存储元素-存储节点''~的映射关系，而分布式终端系统则需要一个相对可靠的
%综合结构化和非结构化，在存储过程引入结构化的内容，而在查询过程使用非结构化的泛洪手段
%因为这样快速且不会对网络造成显著冲击(需要论证)，结构化的优点是有组织?
%半无中心 (所有高级节点成为中心) 与半结构化
